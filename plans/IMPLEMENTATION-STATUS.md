...
## REMAINING
- **Plan 03 Feature F — Scheduled narrative briefs** (§F): daily pod / weekly project / exec brief on a schedule via `ask_service`+`portfolio_feed_service`+LLM. Large: new workflow + cadence/recipient config, mirrored in both engines; recipient resolution can reuse the pod escalation-contact pattern from Feature E.
- **Plan 03 Feature E — deferred nicety:** Admin UI for the pod escalation-contacts endpoints (backend + client types are in place; no frontend form yet).
