...
Alembic migrations through `0027_backfill_age_graph_mirror`. **Next free number: `0028`.**
...
- **Plan 04 ops bucket:** §4a engine consolidation (keep both engines by decision); §4e coverage-gate extension to `infra`/`api`; §4e GitLab-provider doc/config reconciliation. Scratch-doc cleanup is done via `docs/lld/phase-1/check-in-conversation-model.md`, `docs/ops/slack-setup.md`, and `docs/ops/uat-runbook.md`.
