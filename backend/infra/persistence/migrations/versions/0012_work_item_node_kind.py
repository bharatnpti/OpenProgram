"""Add work item graph node kind."""

from __future__ import annotations

from alembic import op

revision = "0012_work_item_node_kind"
down_revision = "0011_workstream_node_kind"
branch_labels = None
depends_on = None

_ALL_NODE_KINDS = (
    "'program', 'project', 'workstream', 'sprint', 'repo', 'pod', 'developer', 'task', 'work_item'"
)
_OLD_NODE_KINDS = "'program', 'project', 'workstream', 'sprint', 'repo', 'pod', 'developer', 'task'"
...
def downgrade() -> None:
    op.execute(
        ...
        WHERE (tenant_id, from_node_id) IN (
            SELECT tenant_id, id FROM graph_nodes WHERE kind = 'work_item'
        )
    ...
    op.execute("DELETE FROM node_statuses WHERE entity_kind = 'work_item';")
    op.execute("DELETE FROM graph_nodes WHERE kind = 'work_item';")
    op.execute("ALTER TABLE graph_nodes DROP CONSTRAINT IF EXISTS graph_nodes_kind_check;")
    op.execute(
        f"""
        ALTER TABLE graph_nodes
